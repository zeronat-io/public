# =============================================================================
# Example: Single ZeroNAT Per Availability Zone
# =============================================================================
#
# Topology:
#
...
#                                └─────┘
#
# - Two independent ZeroNAT single-mode instances, one per AZ.
# - Each has its own private route table — traffic stays in-AZ.
# - No HA/failover between them — if one dies, that AZ loses NAT.
# - Use case: cost-optimized, AZ-local NAT without cross-AZ data transfer.
# =============================================================================
...
resource "aws_vpc" "this" {
  ...
  tags = { Name = "zeronat-single-per-az" }
}
...
# --- Public subnets (ZeroNAT instances live here) ---
...
resource "aws_subnet" "public_a" {
  ...
  tags = { Name = "zeronat-single-per-az-public-a" }
}
...
resource "aws_subnet" "public_b" {
  ...
  tags = { Name = "zeronat-single-per-az-public-b" }
}
...
resource "aws_subnet" "private_a" {
  ...
  tags = { Name = "zeronat-single-per-az-private-a" }
}
...
resource "aws_subnet" "private_b" {
  ...
  tags = { Name = "zeronat-single-per-az-private-b" }
}
...
# --- Public route table: 0.0.0.0/0 → IGW (shared by both public subnets) ---
...
resource "aws_route_table" "public" {
  ...
  tags = { Name = "zeronat-single-per-az-public" }
}
...
# --- Private route tables: one per AZ, 0.0.0.0/0 managed by each ZeroNAT module ---
...
# =============================================================================
# ZeroNAT Modules — One Single-Mode Instance Per AZ
# =============================================================================
...
module "nat_a" {
  ...
  # Only controls the private route table in AZ-a
...
module "nat_b" {
  ...
  # Only controls the private route table in AZ-b
...
resource "aws_instance" "test_a" {
  ...
  tags = { Name = "zeronat-single-per-az-test-a" }
}
...
resource "aws_instance" "test_b" {
  ...
  tags = { Name = "zeronat-single-per-az-test-b" }
}

# --- Test instance security group ---

resource "aws_security_group" "test" {
  name        = "zeronat-single-per-az-test"
  description = "Test instances - egress all, SSH from VPC"
  vpc_id      = aws_vpc.this.id

  tags = { Name = "zeronat-single-per-az-test" }
}
